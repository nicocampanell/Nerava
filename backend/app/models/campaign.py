...
class Campaign(Base):
    ...
    )  # charging_network, hotel, city, oem, merchant, internal
    ...
    # --- Offer URL (for partner/sponsor offers that link to external pages) ---
    offer_url = Column(String(500), nullable=True)  # e.g., "https://evject.com/discount/nerava26"
    ...
    @validates("offer_url")
    def validate_offer_url(self, key, url):
        if url is None:
            return url
        url = url.strip()
        if not url:
        ...
        if parsed.scheme not in ("http", "https"):
            raise ValueError(
                f"offer_url must use an explicit http or https scheme, got {parsed.scheme!r}"
            )
        return url
