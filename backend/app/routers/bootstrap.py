...
async def bootstrap_asadas_party(
    ...
    try:
        ...
        if place_details_data:
            ...
            if not photo_urls_list:
                ...
                if photos:
                    ...
                    for i in range(max_photos):
                        photo = photos[i]
                        photo_ref = (
                            photo.get("name", "").replace("places/", "").split("/photos/")[-1]
                        )
                        if photo_ref:
                            try:
                                ...
                                logger.warning(
                                    f"Failed to get photo URL {i+1} for {merchant.name}: {e}"
                                )
            ...
            merchant.user_rating_count = place_details_data.get(
                "user_rating_count"
            ) or place_details_data.get("userRatingCount")
            merchant.price_level = place_details_data.get("price_level") or place_details_data.get(
                "priceLevel"
            )

            # Get formatted address
            formatted_address = place_details_data.get(
                "formattedAddress"
            ) or place_details_data.get("address")
        ...
        if not primary_email:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Primary merchant email is required"
            )
        ...
        merchant_account = (
            db.query(MerchantAccount).filter(MerchantAccount.owner_user_id == user.id).first()
        )

        if not merchant_account:
            merchant_account = MerchantAccount(id=str(uuid.uuid4()), owner_user_id=user.id)
        ...
        domain_merchant = (
            ...
            .filter(
                DomainMerchant.name == merchant.name, DomainMerchant.zone_slug == "asadas_party"
            )
            .first()
        )
        ...
        if domain_merchant.status != "active":
            logger.error(
                f"Domain merchant {domain_merchant.id} status is {domain_merchant.status}, expected active"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.info(
            f"QR token generated for domain merchant {domain_merchant.id}: {qr_token[:8]}... (status: {domain_merchant.status})"
        )
...
<p><small>If you didn't request this link, you can safely ignore this email.</small></p>
        ...
        magic_link_sent = email_sender.send_email(
            to_email=primary_email, subject=subject, body_text=body_text, body_html=body_html
        )
        ...
        charger_merchant = (
            ...
            .filter(
                ChargerMerchant.charger_id == charger.id, ChargerMerchant.merchant_id == merchant.id
            )
            .first()
        )

        distance_to_charger = haversine_m(
            request.charger_lat, request.charger_lng, merchant.lat, merchant.lng
        )
        ...
        if not charger_merchant:
            ...
            charger_merchant.exclusive_description = (
                "Show your pass for a free beverage during Happy Hour"
            )
        ...
        for place in nearby_places:
            ...
            distance_to_charger_m = haversine_m(
                request.charger_lat, request.charger_lng, place_lat, place_lng
            )
            ...
            if not photo_urls_list:
                logger.info(
                    f"Skipping merchant {place_name} (place_id: {place_id}) - no photos available"
                )
                continue
        ...
        if seeded_count < min_merchants:
            logger.info(
                f"Only {seeded_count} merchants from Google Places, adding static fallback merchants"
            )
            static_merchants = [
                {
                    "name": "Taco Bell",
                ...
                {
                    "name": "McDonald's",
                    "address": "503 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3849,
                    "lng": -97.6899,
                },
                {
                    "name": "Starbucks",
                    "address": "504 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3851,
                    "lng": -97.6901,
                },
                {
                    "name": "Pizza Hut",
                    "address": "505 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3853,
                    "lng": -97.6903,
                },
                {
                    "name": "Burger King",
                    "address": "506 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3855,
                    "lng": -97.6905,
                },
                {
                    "name": "Wendy's",
                    "address": "507 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3857,
                    "lng": -97.6907,
                },
                {
                    "name": "Dunkin'",
                    "address": "508 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3859,
                    "lng": -97.6909,
                },
                {
                    "name": "KFC",
                    "address": "509 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3861,
                    "lng": -97.6911,
                },
                {
                    "name": "Domino's",
                    "address": "510 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3863,
                    "lng": -97.6913,
                },
                {
                    "name": "Chipotle",
                    "address": "511 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3865,
                    "lng": -97.6915,
                },
                {
                    "name": "Papa John's",
                    "address": "512 W Canyon Ridge Dr, Austin, TX 78753",
                    "lat": 30.3867,
                    "lng": -97.6917,
                },
            ]
            ...
            for static_merchant in static_merchants:
                ...
                merchant_id_slug = (
                    static_merchant["name"].lower().replace("'", "").replace(" ", "_")
                )
                seeded_merchant_id = f"m_static_{merchant_id_slug}"

                # Check if already exists
                seeded_merchant = (
                    db.query(Merchant).filter(Merchant.id == seeded_merchant_id).first()
                )
        ...
        return AsadasPartyResponse(
            ...
            primary_merchant={"id": merchant.id, "qr_token": qr_token},
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Bootstrap failed: {str(e)}"
        )


@router.get("/debug/merchant-by-qr/{token}")
async def debug_merchant_by_qr(
    token: str, db: Session = Depends(get_db), bootstrap_key: str = Depends(verify_bootstrap_key)
):
    """
    ...
    merchant = db.query(DomainMerchant).filter(DomainMerchant.qr_token == token).first()

    if not merchant:
        return {"found": False, "token": token, "message": "No merchant found with this QR token"}
    ...
    return {
        ...
        "qr_last_used_at": (
            merchant.qr_last_used_at.isoformat() if merchant.qr_last_used_at else None
        ),
...
async def get_bootstrap_schema():
    ...
    return {
        ...
        "response": {
            "ok": True,
            "cluster_id": "<uuid>",
            "primary_merchant": {"id": "<merchant_id>", "qr_token": "<qr_token>"},
...
async def bootstrap_trigger_seed(
    ...
    if seed_type == "chargers":
        ...
        if lat is not None and lng is not None:
            ...
            logger.info(
                f"[TriggerSeed] Targeting {len(chargers_override)} chargers near ({lat}, {lng}) r={radius_km}km"
            )
