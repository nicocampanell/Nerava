...
async def seed_city(city: str, bbox_str: str, categories: list = None):
    ...
    try:
        ...
        for charger_data in charger_data_list:
            # Check if exists
            existing = (
                db.query(Charger).filter(Charger.external_id == charger_data.external_id).first()
            )
        ...
        for charger in charger_objects[:20]:  # Limit to avoid too many API calls
            ...
            for category in categories:
                try:
                    ...
                    logger.info(
                        f"[SeedCity] Charger {charger.id}: Searching category '{category}' -> types {place_types} keyword='{keyword}'"
                    )
                    ...
                    logger.info(
                        f"[SeedCity] Charger {charger.id} category '{category}': Found {len(places)} places"
                    )

                    if not places:
                        logger.debug(
                            f"[SeedCity] No places found for charger {charger.id} category {category}"
                        )
                        continue
                    ...
                    for place in places:
                        ...
                        merchant = (
                            db.query(Merchant)
                            .filter(Merchant.external_id == place.place_id)
                            .first()
                        )
                        ...
                        if not merchant:
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"Could not get place details for {place.place_id}: {e}"
                                )
                    ...
                    logger.error(
                        f"Error processing category {category} for charger {charger.id}: {e}",
                        exc_info=True,
                    )
        ...
        db.commit()
        logger.info(
            f"Seed complete: {total_merchants} merchants, {total_links} charger-merchant links"
        )
...
def main():
    ...
    parser.add_argument(
        "--bbox", required=True, help="Bounding box: min_lat,min_lng,max_lat,max_lng"
    )
    parser.add_argument(
        "--categories",
        nargs="+",
        default=["coffee", "food", "groceries", "gym"],
