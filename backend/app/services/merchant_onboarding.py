...
def validate_merchant_location(db: Session, zone_slug: str, lat: float, lng: float) -> Zone:
    """
    ...
    distance = haversine_m(zone.center_lat, zone.center_lng, lat, lng)
...
def check_duplicate_merchant(
    ...
    if google_place_id:
        existing = (
            ...
            .filter(DomainMerchant.google_place_id == google_place_id)
            .first()
        )
        if existing:
            return existing
    ...
    if name and lat is not None and lng is not None:
        ...
        candidates = db.query(DomainMerchant).filter(DomainMerchant.name.ilike(f"%{name}%")).all()

        for candidate in candidates:
            distance = haversine_m(candidate.lat, candidate.lng, lat, lng)
...
async def onboard_merchant_via_square(
    ...
    merchant = (
        db.query(DomainMerchant)
        .filter(DomainMerchant.square_merchant_id == square_result.merchant_id)
        .first()
    )
    ...
    if merchant:
        ...
        db.flush()
        logger.info(
            f"Created new merchant {merchant.id} for Square merchant {square_result.merchant_id}"
        )
    ...
    try:
        location_stats = await fetch_square_location_stats(
            square_result.access_token, square_result.location_id
        )
