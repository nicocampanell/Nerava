...
async def get_merchant_details(
    ...
    if merchant.id and not merchant.id.startswith("m_mock_"):
        perk = (
            ...
            .filter(MerchantPerk.merchant_id == merchant.id, MerchantPerk.is_active == True)
            .first()
        )
    ...
    if perk:
        perk_info = PerkInfo(
            ...
            or "Show your Nerava app to redeem your reward while you charge.",
        )
    ...
    link = None
    if merchant.id and not merchant.id.startswith("m_mock_"):
        link = db.query(ChargerMerchant).filter(ChargerMerchant.merchant_id == merchant.id).first()
    ...
    if not perk_info and link and link.exclusive_title:
        perk_info = PerkInfo(
            ...
            badge="Exclusive",
            description=link.exclusive_description
            or "Show your Nerava app to redeem your reward while you charge.",
        )
    ...
    if (
        merchant.id
        and not merchant.id.startswith("m_mock_")
        and not getattr(merchant, "primary_photo_url", None)
        and getattr(merchant, "place_id", None)
    ):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"[MerchantDetails] Failed to refresh merchant after enrichment: {refresh_error}"
                )
        except Exception as e:
            logger.warning(
                f"[MerchantDetails] Failed to enrich photos for merchant {merchant.id}: {e}",
                exc_info=True,
            )
    ...
    if merchant.id and not merchant.id.startswith("m_mock_"):
        ...
        vote_counts = (
            ...
            .filter(AmenityVote.merchant_id == merchant.id)
            .group_by(AmenityVote.amenity, AmenityVote.vote_type)
            .all()
        )
    ...
    merchant_info = MerchantInfo(
        ...
        category=category,
        photo_url=getattr(
            merchant, "primary_photo_url", None
        ),  # Only Google Places photos, no fallback
        photo_urls=(
            getattr(merchant, "photo_urls", None) if getattr(merchant, "photo_urls", None) else []
        ),  # Ensure array, not None
    ...
    moment_info = MomentInfo(
        label=moment_label, distance_miles=round(distance_miles, 1), moment_copy=moment_copy
    )
    ...
    actions_info = ActionsInfo(add_to_wallet=True, get_directions_url=directions_url)
    ...
    try:
        ...
        raw_state = get_merchant_reward_state(
            ...
            merchant_id=(
                merchant.id if merchant.id and not merchant.id.startswith("m_mock_") else None
            ),
