...
def score_hub(hub: Dict[str, Any], user_id: str, context: Dict[str, Any]) -> float:
    ...
    distance_km = haversine_m(user_lat, user_lng, hub_lat, hub_lng) / 1000
...
def score_perk(perk: Dict[str, Any], user_id: str, context: Dict[str, Any]) -> float:
    ...
    if (
        "coffee" in perk.get("name", "").lower()
        and 6 <= hour <= 10
        or "lunch" in perk.get("name", "").lower()
        and 11 <= hour <= 14
    ):
        base_score += 30
...
def rank_hubs_and_perks(
    user_id: str, lat: float, lng: float, hubs: List[Dict[str, Any]], perks: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
    ...
    for hub in hubs:
        ...
        hub_scores.append(
            {"hub": hub, "score": score, "reason": "High value hub with good rewards and proximity"}
        )
